...
import useGalleryPhotoUrl from "@/hooks/useGalleryPhotoUrl";
...
export default function GalleryPageSingle({ gallery }: GalleryPageSingleProps) {
  ...
  const [photos, setPhotos] = useState<GalleryPhoto[][]>(
    splitPhotoListInColumns(gallery.photos, 3, 50),
  );
  const photosRef = useRef<HTMLDivElement>(null);
  const { getGalleryPhotoUrl } = useGalleryPhotoUrl();
  ...
  return (
    <div className="gallery-page">
      <div className="gallery-title">
        ...
        {gallery.subtitle && <div className="subtitle">{gallery.subtitle}</div>}
      </div>
            ...
            {gallery.categories.map((category, idx) => {
              ...
              return (
                <div key={gallery.title + category} style={style}>
                  <a href={"/reveries/" + categories.join("/")}>{category}</a>
                </div>
              );
            })}
          </div>
            ...
            <div className="photos-column" key={"column-" + index}>
              {column.map((photo: GalleryPhoto) => (
                <div className="photo" key={photo.path}>
                  <Lightbox
                    img={getGalleryPhotoUrl(photo, gallery)}
                    images={gallery.photos.map((p) =>
                      getGalleryPhotoUrl(p, gallery),
                    )}
                  />
